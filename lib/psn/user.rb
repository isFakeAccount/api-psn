module PSN
  class User
    ...
    USERS_ENDPOINT     ||= 'https://us-prof.np.community.playstation.net/userProfile/v1/users/%s'
    ...
    def info
      request = PSN::Client.new(base_uri: format(USERS_ENDPOINT, identity))
    ...
    def games(limit: 100, offset: 0)
      request = PSN::Client.new(base_uri: format(GAMES_ENDPOINT, identity))
    ...
    def communities(limit: 25, offset: 0)
      request = PSN::Client.new(base_uri: COMMUNITY_ENDPOINT)
    ...
    def activity(page: 0, offset: 0, size: 25, include_comments: true)
      request = PSN::Client.new(base_uri: format(ACTIVITY_ENDPOINT, identity))
